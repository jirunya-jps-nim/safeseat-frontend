...
function TrackingContent() {
    ...
    try {
      ...
      if (res.data.success) {
        if (res.data.data.requestType === 'user') {
          // หากรหัสบริการเป็นงานฝั่ง User ให้ส่งผู้ใช้ไปยังเพจติดตามงานทั่วไป (/trip)
    ...
    return (
      <div style={s.pageCenter}>
        <div style={{ textAlign: 'center', backgroundColor: '#fff', padding: 40, borderRadius: 20, border: '1px solid #e2e8f0' }}>
          ...
          <button style={s.btnPrimary} onClick={goHome}>กลับหน้าแรก</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={s.page}>
      <nav style={s.navbar}>
        <div onClick={goHome} style={{ ...s.navLeft, cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, width: '100%' }}>
          ...
          <div style={{ ...s.card, padding: '32px 40px' }} className="tracking-card">
            ...
            <div className="details-grid" style={{ display: 'grid', gridTemplateColumns: '1.45fr 1.75fr 0.8fr', gap: 40, alignItems: 'start' }}>
              ...
              {/* Column 2: Middle - Driver Profiles */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {(reqData.leader || requeststatus !== 'รอคนขับ') ? (
                  ...
                  isRequestTimeout ? (
                    <div style={{
                      ...
                      <div style={{ fontWeight: 700, fontSize: 15 }}>ไม่มีคนขับรับงาน</div>
                      <div style={{ fontSize: 12, color: '#b91c1c', lineHeight: 1.4 }}>ขออภัยครับ ไม่มีคนขับพร้อมรับงานในพื้นที่ภายในระยะเวลาที่กำหนด</div>
                    </div>
                  ) : (
                    <div style={{
                      display: 'flex',
                      ...
                      <div style={{ fontWeight: 700, fontSize: 15 }}>กำลังจัดหาคู่หูคนขับ...</div>
                      <div style={{ fontSize: 12, color: '#b45309', lineHeight: 1.4 }}>เมื่อมีทีมคนขับตอบรับงาน ระบบจะแสดงข้อมูลคนขับและช่องทางติดต่อทันที</div>
                    </div>
                  )
                )}
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
                    ...
                    >
                      {copied ? '✅ คัดลอกสำเร็จ!' : '📋 คัดลอกลิงก์ติดตาม'}
                    </button>
                  </div>
                </div>
              </div>
            ...
            {isCancelled && (
              <div style={{ marginTop: 20, padding: '16px', backgroundColor: '#fef2f2', color: '#dc2626', borderColor: '#fecaca', borderRadius: 12, fontWeight: 700, textAlign: 'center' }}>
